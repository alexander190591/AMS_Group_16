...
word file_write(file_descriptor_t* fd, char* write_str, word length)
{
  ...
  if(!ReadBlock(g_block_buf, sec_addr)) { return 0; }
  ...
  if (!WriteBlock(g_block_buf, sec_addr)) { return 0; }
  ...
  if (length > sec_bytes_left)
  {
    ...
    if (!WriteBlock(g_block_buf, get_sec_addr(fd->cluster, fd->position)))
    { 
      return 0; 
    }
...
word file_read(file_descriptor_t *fd, char *buf, word length)
{
  ...
  if (length > sec_bytes_left)
  {
    if (!ReadBlock(g_block_buf, get_sec_addr(fd->cluster, fd->position)))
    { 
      return 0; 
    }
