...
struct AirSensor
{
	...
	uint8_t _dataBitCounter;										// Variable for counting amount of data collected.
	uint8_t _newDataAvailable;										// Variable set when reading data from DHT11 is done.
	uint8_t _byteNumber;											// Variable for keeping track of what byte is currently written to in _sensorData[].
	uint8_t _bitNumber;												// Variable for keeping track of what bit is currently written to in _sensorData[].
	uint8_t _responseSignalReceived;								// Variable set when response signal from DHT11 has been received.
	...
	uint8_t (*getValues)(struct AirSensor*);							// Function pointer for getValues 'method'.
...
uint8_t getValues(struct AirSensor*);									// Reads values from the DHT11 sensor and stores it in _temperature and _humidity.
