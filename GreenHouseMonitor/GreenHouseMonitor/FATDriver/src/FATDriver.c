 ...
 */ 
 ...
 {
	SendDebugMessage("In initFAT now!");
	 byte i;
...
//		 pmsg("read timed out");
		 #endif
		 SendDebugMessage("read timed out!");
		 return 0;
	 }
	 SendDebugMessage("first 512 bytes read!");
...
//		 pmsg("drive is not FAT16!\r\n");
		 #endif
		 SendDebugMessage("drive is not FAT16!");
		 return 0;
	 }
	 SendDebugMessage("drive is FAT16! continuing.");
...
//	 pmsg("partition_addr is: "); disdword(partition_addr); pmsg("\r\n");
	 #endif
	 if (!ReadBlock(buf, partition_addr))
	 {
		 #if DEBUG
//		 pmsg("couldn't read start of partition!\r\n");
		 #endif
		 SendDebugMessage("couldn't read start of partition!");
		 return 0;
	 }
	 SendDebugMessage("Start of partition read!");
	 
	 prtn_set_addr(buf, partition_addr);
	 SendDebugMessage("FAT Initialized!");
	 return 1;
 }
...
//	 pmsg("Data starts at: 0x"); disdword(prtn.data_addr); pmsg("\r\n");
 ...
 dword get_empty_dir_entry(byte* buf)
 {
	 ...
	 for (i=0;i<32;i++)
	 {
		 b = ReadBlock(buf, prtn.root_dir_addr + i*512);
		 for(j=0;j<16;j++)
		 {
...
//	 pmsg("Failed to find empty directory entry!\r\n");
 ...
 word get_unused_cluster(byte* buf)
 {
	 ...
	 {
		 b = ReadBlock(buf, prtn.FAT_addr + i*512);
		 for(j=0;j<512/2;j++)
		 {
...
//		 pmsg("Couldn't find empty direntry!\r\n");
 ...
 byte write_dir_entry(dir_entry_t* de, byte* buf)
 {
	 ...
	 if(!WriteBlock(buf, de_sec_start))
	 {
		 #if DEBUG
...
//		 pmsg("Problem clearing FAT!\r\n");
 ...
 word next_clus(word clus, byte *buf)
 {
	 ...
	 if(!ReadBlock(buf, addr - addr % 512))
	 {
		 #if DEBUG
